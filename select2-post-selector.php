...
/**
 * @wordpress-plugin
 * Plugin Name:       Select2 Post Selector - Modified
 ...
 * Text Domain:       select2-post-selector-modified
...
class S2PS_Post_Select_Instance {
	 ...
	 */
	function display() {
		$processed_item_ids = get_option( $this->meta_key );
	?>
		<p>
	...
	public static function post_lookup() {
		...
		foreach ( $posts as $this_post ) {
			...
			$result[] = array(
				'id' => $id,
				'title' => $post_title,
				'thumbnail' => get_the_post_thumbnail_url( $this_post ),
				'excerpt' => substr( strip_tags( $this_post->post_content ), 0, 250 ),
			);
		}
